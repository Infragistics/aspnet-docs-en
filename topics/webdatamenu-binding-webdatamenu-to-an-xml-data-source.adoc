...
[start=10]
. In the Property window for WebDataMenu, set the  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.framework.data.hierarchicaldataboundcontrolmain~datasourceid.html[DataSourceID]"]  property to be the id of the XmlDataSource component.
[start=11]
...
[start=13]
. Select Customer node and click the Add Databinding button. This will add the item to the Selected databindings area. Set the  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.navigationcontrols.navitembinding~textfield.html[TextField]"]  property to CustomerName by clicking the dropdown combo in the property window so that CustomerName will be the display text of the item. Similarly, set the  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.navigationcontrols.navitembinding~valuefield.html[ValueField]"]  property to CustomerID .
