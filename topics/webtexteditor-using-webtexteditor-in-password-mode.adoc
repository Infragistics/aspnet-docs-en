...
To use WebTextEditor in password mode you must set the control’s  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.editorcontrols.webtexteditor~textmode.html[TextMode]"]  property to Password. You can set the TextMode property to Password either by using the Microsoft® Visual Studio® Property Window or by using the following code:
