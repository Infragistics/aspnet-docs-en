...
[start=1]
. Select WebHtmlEditor on the design surface and search through the Properties Window for the  pick:[asp-net="link:{ApiPlatform}webui.webhtmleditor{ApiVersion}~infragistics.webui.webhtmleditor.webhtmleditor~tabstrip.html[TabStrip]"]  property. Expand the property to view the list of sub-properties, which allow you to customize the tab strip region.
