...
The Summary Row behavior has a property called link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.gridcontrols.summaryrow~compactrendering.html[CompactRendering], which specifies how the summaries are rendered. There are three available options:
...
When Summary Row and Filtering behaviors are enabled, the summary value can be calculated for the filtered data. There is a property called link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.gridcontrols.summaryrow~enablesummariesfilter.html[EnableSummariesFilter], which specifies how the summaries are calculated:
