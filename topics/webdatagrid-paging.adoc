...
[start=4]
. In the properties, set  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.gridcontrols.paging~pagesize.html[PageSize]"]  to 10, increasing the number of records per page to 10\.
[start=5]
...
[start=7]
. In the property window, check that the  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.gridcontrols.webdatagrid~enableajax.html[EnableAjax]"]  property of WebDataGrid is set to True. If not, set it to True. WebDataGrid is ready to page data using AJAX.
