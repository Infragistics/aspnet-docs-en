...
[NOTE]
====
When enabling  pick:[asp-net="link:{ApiPlatform}web{ApiVersion}~infragistics.web.ui.gridcontrols.rowdeleting.html[RowDeleting]"]  you are also prompted to add the Selection behavior. If you choose to do so, Selection will be enabled with the and CellClickAction properties set to Single and Row, respectively.
====
