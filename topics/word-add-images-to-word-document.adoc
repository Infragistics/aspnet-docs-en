...
Infragistics® Word library supports two kinds of images that can be added to a new Word document using the link:{ApiPlatform}webui.documents.io{ApiVersion}~infragistics.documents.word.worddocumentwriter.html[WordDocumentWriter] streamer object: anchored and inline image (also referred to here as a picture).
...
To add inline pictures to a paragraph, you use the the link:{ApiPlatform}webui.documents.io{ApiVersion}~infragistics.documents.word.worddocumentwriter~addinlinepicture.html[AddInlinePicture] method of the WordDocumentWriter object.
